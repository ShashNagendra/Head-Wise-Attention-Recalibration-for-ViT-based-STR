...
__all__ = [
    'vitstr_tiny_patch16_224_withHeadSE', 
    'vitstr_small_patch16_224_withHeadSE', 
    'vitstr_base_patch16_224_withHeadSE',
...
class HeadSEGate(nn.Module):
    def __init__(self, num_heads, reduction=4):
        super().__init__()
        self.fc1 = nn.Linear(num_heads, max(1, num_heads // reduction))
        self.fc2 = nn.Linear(max(1, num_heads // reduction), num_heads)

    def forward(self, heads):
        # heads: (B, L, H, D_head)
        z = heads.mean(dim=(1,3))  # pooled (B, H)
        w = F.relu(self.fc1(z))
        w = torch.sigmoid(self.fc2(w))  # (B, H)
        return heads * w.unsqueeze(1).unsqueeze(-1)


from timm.models.vision_transformer import Attention

class SEAttention(Attention):
    """
    timm Attention + SE-style head recalibration
    """
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.head_se = HeadSEGate(num_heads)
    ...
    def forward(self, x):
        ...
        # ---- per-head output ----
        heads = attn @ v                    # (B, H, N, D)
        heads = heads.permute(0, 2, 1, 3)   # (B, N, H, D)

        # ---- SE over heads ----
        heads = self.head_se(heads)
...
class ViTSTR(VisionTransformer):
    ...
    def replace_attention_with_se(self):
        for blk in self.blocks:
            old_attn = blk.attn
            blk.attn = SEAttention(
            ...
            blk.attn.qkv.weight.data.copy_(old_attn.qkv.weight.data)
            blk.attn.proj.weight.data.copy_(old_attn.proj.weight.data)

            if old_attn.qkv.bias is not None:
                blk.attn.qkv.bias.data.copy_(old_attn.qkv.bias.data)
                blk.attn.proj.bias.data.copy_(old_attn.proj.bias.data)
...
@register_model
def vitstr_tiny_patch16_224_withHeadSE(pretrained=False, **kwargs):
    kwargs['in_chans'] = 1
    model = ViTSTR(patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True, **kwargs)
    model.replace_attention_with_se()
...
@register_model
def vitstr_small_patch16_224_withHeadSE(pretrained=False, **kwargs):
    kwargs['in_chans'] = 1
    model = ViTSTR(patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, **kwargs)
    model.replace_attention_with_se()
    model.default_cfg = _cfg(
...
@register_model
def vitstr_base_patch16_224_withHeadSE(pretrained=False, **kwargs):
    kwargs['in_chans'] = 1
    model = ViTSTR(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, **kwargs)
    model.replace_attention_with_se()
    model.default_cfg = _cfg(
